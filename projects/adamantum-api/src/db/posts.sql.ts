...
export const UPDATE_POST: string = 'UPDATE posts SET postTitle = ?, postContent = ? , categoryId = ? WHERE postId = ?';
...
export const GET_POSTS_LIST: string = 'WITH RECURSIVE category_hierarchy AS (SELECT c1.categoryId, c1.categoryName, c1.categoryParent FROM categories c1 WHERE c1.categoryParent IS NULL UNION ALL SELECT c2.categoryId, c2.categoryName, c2.categoryParent FROM categories c2 INNER JOIN category_hierarchy ch ON ch.categoryId = c2.categoryParent) SELECT p.postTitle, p.postId, p.fullPath, c.categoryId, c.categoryName, ch.categoryName AS parentCategoryName , c.categoryParent AS categoryParentId FROM posts p LEFT JOIN categories c ON p.categoryId = c.categoryId LEFT JOIN category_hierarchy ch ON c.categoryParent = ch.categoryId WHERE p.deletedAt IS NULL AND c.deletedAt IS NULL;';
